...
def test_health_returns_503_without_leaking_database_errors(tmp_path) -> None:
    app = build_test_app(f"sqlite:///{tmp_path / 'health.db'}")

    class BrokenSession:
        def execute(self, *_args, **_kwargs):
            raise RuntimeError("postgresql://user:secret@example.invalid/afran")
    ...
    with TestClient(app) as client:
        ...
        assert response.json()["details"] == {"database_error": "RuntimeError"}
