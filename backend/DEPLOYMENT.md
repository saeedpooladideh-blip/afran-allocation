...
- SQLite startup verifies that the mounted database directory is writable before
  running Alembic.
- The scheduler is rejected when `WEB_CONCURRENCY` is greater than one, preventing
  duplicate crawls inside a single container.
- Database health failures return HTTP `503` without exposing connection details.
